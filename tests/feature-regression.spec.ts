...
test.describe('modern NetEase API integration', () => {
  ...
  test('exposes daily history, recommendation feedback, styles, new works, and cloud management', () => {
    ...
    expect(insights).toContain("const cloudBatchRangeStart = ref('')")
    expect(insights).toContain("const cloudBatchRangeEnd = ref('')")
    expect(insights).toContain('cloudTracks.value.slice(from - 1, to)')
