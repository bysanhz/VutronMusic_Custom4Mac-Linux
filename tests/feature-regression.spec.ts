...
test.describe('library hydration and stable rendering', () => {
  test('repairs nullable persisted liked state and keeps the compact liked-song preview bounded', () => {
    ...
    expect(dataStore).toContain('trackIDs.slice(0, 40)')
    expect(dataStore).toContain('Promise.allSettled(')
    ...
    expect(library).toContain('class="liked-preview-grid"')
    expect(library).toContain('v-for="track in likedSongsPreview"')
