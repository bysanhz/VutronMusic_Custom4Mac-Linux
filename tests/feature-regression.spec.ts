...
test.describe('heart mode history-aware recommendations', () => {
  ...
  test('persists a cross-session cooldown and supplements candidates with multiple seeds', () => {
    ...
    expect(settings).not.toContain('settings.heartModeProfile.title')
    expect(settings).not.toContain('v-model="selectedHeartModeMode"')
    expect(settings).not.toContain("heartModeProfile.mode === 'custom'")
    expect(settings).not.toContain("updateHeartModeAdvanced('maxSameArtistDistance', $event)")
    expect(settings).not.toContain("updateHeartModeAdvanced('maxSameSeedDistance', $event)")
...
test.describe('desktop feature integration', () => {
  ...
  test('includes queue-end sleep actions and preset editing tools', () => {
    ...
    expect(presetTransfer).toContain('vutronmusic-osd-template-bundle')
    expect(presetTransfer).toContain('vutronmusic-osd-preset')
    expect(presetTransfer).toContain('导出模版')
    expect(presetTransfer).toContain('导入模版')
    expect(presetTransfer).toContain('templates')
    expect(presetTransfer).toContain('const askConflictResolution =')
    expect(presetTransfer).toContain("replaceOne: '替换'")
    expect(presetTransfer).toContain("skipOne: '跳过'")
    expect(presetTransfer).not.toContain("replaceAll:")
    expect(presetTransfer).not.toContain("skipAll:")
  ...
  test('restores auto-hidden Windows desktop lyrics and keeps idle hiding active', () => {
    ...
    expect(osdPreload).toContain('let osdLocked =')
    expect(osdPreload).toContain('let mouseInside = false')
    expect(osdPreload).toContain('const restoreRootVisibility = () => {')
    expect(osdPreload).toContain('const scheduleLockedAutoHide = () => {')
    expect(osdPreload).toContain('if (!osdLocked || !mouseInside) return')
    expect(osdPreload).toContain('showButtonWhenLock')
    expect(osdPreload).toContain("ipcRenderer.on('mouseInWindow', handleMouseInWindow)")
    expect(osdPreload).toContain("ipcRenderer.on('set-isLock', handleSetIsLock)")
    expect(osdPreload).toContain('osdLocked = Boolean(value)')
    ...
    expect(osdPreload).toContain("root.addEventListener('mouseleave', () => {")
    expect(osdPreload).toContain("root.addEventListener('mousemove', () => {")
    expect(osdPreload).not.toContain('const lockClassObserver = new MutationObserver')
  })
  ...
  test('fully restores desktop lyric interaction after unlock', () => {
    ...
    expect(ipcs).toContain("lyricWin?.webContents.send('set-isLock', Boolean(value))")
    expect(ipcs).not.toContain('lrc.toggleMouseIgnore()')

    expect(mainIndex).not.toContain('this.toggleMouseIgnore()\n      this.checkOsdMouseLeave()')
    expect(preload).toContain("'updateOsdState'")
    expect(preload).toContain("'set-isLock'")
  ...
  test('supports custom cache limits and keeps settings controls aligned', () => {
    ...
    expect(settingsFluidLayout).toContain('--settings-content-right-gutter: 24px;')
    expect(settingsFluidLayout).toContain(
      'padding-right: var(--settings-content-right-gutter) !important;'
    )
    expect(settingsFluidLayout).toContain(
      'width: calc(100% - var(--settings-content-right-gutter)) !important;'
    )
    expect(settingsFluidLayout).toContain(
      '#app .system-settings :is(.update-release, .diagnostic-panel)'
    )
  ...
  test('uses one shared bottom lock and unlock action for normal and compact desktop lyrics', () => {
    const osd = readSource('src/renderer/views/OSDLyric.vue')
    const header = readSource('src/renderer/components/OsdHeader.vue')
  ...
  test('keeps the desktop lyric drag handle large and dominant over resize hit zones', () => {
    ...
    expect(osd).toContain('width: clamp(112px, 24vw, 180px);')
    expect(osd).toContain('height: 26px;')
    expect(osd).toContain('width: clamp(138px, 22vw, 220px);')
    expect(osd).toContain('height: 28px;')
    expect(osd).toContain('width: 66%;')
    expect(osd).toContain('height: 6px;')
    expect(osd).toContain('opacity: 0.34;')
    expect(osd).toContain('border-radius: 999px;')
    expect(osd).toContain('width: 72%;')
...
test.describe('modern NetEase API integration', () => {
  ...
  test('exposes daily history, recommendation feedback, styles, new works, and cloud management', () => {
    ...
    expect(explore).toContain('class="buttons playlist-filter-bar"')
    expect(explore).toContain('class="panel artist-filter-panel"')
    expect(explore).toContain("'artist-alpha-row': bigCat === '筛选'")
    expect(explore).toContain('grid-template-columns: 72px minmax(0, 1fr);')
    expect(explore).toContain('grid-template-columns: repeat(auto-fit, minmax(42px, 1fr));')
    expect(insights).toContain('const cloudBatchSongIds = ref<string[]>([])')
    expect(insights).not.toContain("const cloudBatchRangeStart = ref('')")
    expect(insights).not.toContain("const cloudBatchRangeEnd = ref('')")
    expect(insights).toContain('const selectedIndexes = cloudTracks.value')
    expect(insights).toContain('cloudTracks.value.slice(from, to + 1)')
    expect(insights).toContain("t('insights.cloud.batchRangeNeedEndpoints')")
    expect(insights).toContain('cloudBatchSongIds.length < 2')
    expect(insights).toContain('const DELETE_CONCURRENCY = 3')
    expect(insights).toContain('@click="removeCloudSongs"')
    expect(insights).toContain('@click="selectCloudBatchRange"')
    expect(insights).not.toContain('@click="removeCloudSong"')
    expect(insights).not.toContain('const removeCloudSong = async')
    expect(insights).toContain('class="cloud-batch-sticky-tools"')
    expect(insights).toContain("t('insights.cloud.singleToolsTitle')")
    expect(insights).toContain("t('insights.cloud.targetIdHint')")
    expect(insights).toContain('class="cloud-rematch-controls"')
    expect(insights).toContain('class="cloud-delete-entry"')
    expect(insights).toContain('class="insights-scroll-top"')
    expect(insights).toContain('icon-class="arrow-up-alt"')
    expect(insights).toContain("const scrollToInsightsTop = (): void => scrollMainTo(0, 'smooth')")
    expect(insights).toContain('position: sticky;')
    expect(insights).toContain('top: 8px;')
    expect(insights).toContain("t('insights.cloud.batchDeleteConfirm', { count: ids.length })")
    expect(insights).toContain('// Keep batch mode open after a successful deletion')
    expect(insights).not.toContain('.cloud-batch-list {\n  max-height: 260px;')
...
test.describe('virtual list and desktop lyric preview stability', () => {
  ...
  test('lays out desktop lyric presets as preview and control panes', () => {
    ...
    expect(shared).toContain('.vutronmusic-template-conflict-overlay {')
    expect(shared).toContain('.vutronmusic-template-conflict-actions {')
  })
  ...
  test('previews the selected preset and keeps the canonical teal/pink palette', () => {
    ...
    expect(transferPreview).toContain("schema: TEMPLATE_BUNDLE_SCHEMA")
    expect(transferPreview).toContain("version: 1")
    expect(transferPreview).toContain("option.value !== CURRENT_SETTINGS_OPTION_ID")
    expect(transferPreview).toContain("sameSettings(currentSettings, template.settings)")
    expect(transferPreview).toContain("const askConflictResolution =")
    expect(transferPreview).toContain("type ConflictResolution = 'replace' | 'skip'")
    expect(transferPreview).toContain(
      "(await askConflictResolution(name)) === 'replace'"
    )
    expect(transferPreview).toContain("await shouldReplaceConflict(builtInConflict.name)")
    expect(transferPreview).toContain("await shouldReplaceConflict(userConflict.name)")
    expect(transferPreview).toContain("replaceOne: '替换'")
    expect(transferPreview).toContain("skipOne: '跳过'")
    expect(transferPreview).not.toContain("replace-all")
    expect(transferPreview).not.toContain("skip-all")
    expect(transferPreview).toContain("refreshUserPresetOptions(nextUsers, previousValue)")
    expect(transferPreview).not.toContain("window.confirm(")
    expect(transferPreview).not.toContain("applySettings(settings)")
  })
})
...
test.describe('OSD fallback palette and nested scroll ergonomics', () => {
  ...
  test('keeps compact cover narrow and restores native nested wheel scrolling', () => {
    ...
    expect(app).toContain('mainScrollFrame = window.requestAnimationFrame(syncMainScrollState)')
    expect(virtualScroll).toContain(
...
test.describe('library hydration and stable rendering', () => {
  test('repairs nullable persisted liked state and keeps the compact liked-song preview bounded', () => {
    ...
    expect(dataStore).toContain('const PREVIEW_SIZE = 6')
    expect(dataStore).toContain('trackIDs.slice(0, 24)')
    expect(dataStore).toContain('detailByID')
    expect(dataStore).not.toContain('trackIDs.slice(0, 40)')
    expect(library).toContain('const libraryData = computed(() =>')
    expect(library).toContain('const likedSongsPreview = computed(() =>')
    expect(library).toContain('songsWithDetails.slice(0, 6)')
    expect(library).toContain('playRandomLyricTrack')
    expect(library).toContain('class="lyric-preview-action"')
    expect(library).not.toContain('class="liked-preview-grid"')
    expect(library).not.toContain(':items="likedSongsPreview"')
    expect(library).toContain('class="liked-preview-list"')
    expect(library).toContain('v-for="(row, rowIndex) in likedPreviewRows"')
    expect(library).toContain('class="liked-preview-row"')
    expect(library).toContain('min-height: 68px;')
    expect(library).not.toContain('new IntersectionObserver(')
    expect(library).not.toContain('applyTabIntersection')
    expect(library.match(/:fixed-column-number="true"/g)?.length).toBeGreaterThanOrEqual(3)
    expect(library.match(/:colunm-number="4"/g)?.length).toBeGreaterThanOrEqual(3)
    expect(library).toContain('-webkit-line-clamp: 2;')
    expect(library).toContain(':global(#app) .library .liked-preview-artist')
    expect(library).toContain('calc(0.8125 * var(--app-global-font-size))')
...
test.describe('local music scanning controls', () => {
  test('exposes a visible rescan action on the local music page', () => {
    ...
    expect(localMusic).toContain("t('contextMenu.reScan')")
    expect(localMusic).toContain("t('toast.scanning')")
    expect(zhHans).toContain('"reScan": "扫描歌曲"')
    expect(zhHant).toContain('"reScan": "掃描歌曲"')
    expect(en).toContain('"reScan": "Scan Tracks"')
  })
})
...
test.describe('native outer-scroll cover rendering', () => {
  test('avoids per-frame JS windowing while skipping offscreen cover work', () => {
    ...
    expect(coverRow).not.toContain('content-visibility: auto;')
    expect(coverRow).not.toContain('contain-intrinsic-size:')
    expect(coverRow).toContain('fixedColumnNumber')
    expect(coverRow).toContain('if (props.fixedColumnNumber)')
    expect(coverBox).toContain('decoding="async"')
    expect(coverBox).toContain('class="cover-loading-spinner"')
    expect(coverBox).toContain('observeCoverVisibility')
    expect(coverBox).not.toContain('imageLoadingTimer')
    expect(coverBox).not.toContain('window.setTimeout(() =>')
  })
})
