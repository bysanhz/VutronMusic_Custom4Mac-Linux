...
test.describe('music insights rendering stability', () => {
  ...
  test('keeps pending local listen time visible until NetEase statistics catch up', () => {
    ...
    expect(view).toContain('reconcileNeteaseRemoteWeekDuration(nextRemoteWeekSeconds)')
    ...
    expect(view).toContain('待同步 +')
    expect(view).toContain('本周（周一至今）')
    expect(view).not.toContain('本机 +')
    expect(pending).toContain("STORAGE_KEY = 'vutronmusic-netease-listen-pending-v1'")
    expect(pending).toContain('lastRemoteWeekSeconds')
    expect(pending).not.toContain('lastRemoteTotalSeconds')
    expect(pending).toContain('pendingNeteaseListenSeconds.value += value')
    expect(pending).toContain('next > lastRemoteWeekSeconds')
    ...
    expect(view).toContain('grid-template-rows: auto 1fr auto;')
