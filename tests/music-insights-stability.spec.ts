...
test.describe('music insights rendering stability', () => {
  ...
  test('keeps pending local listen time visible until NetEase statistics catch up', () => {
    ...
    expect(view).toContain('const displayTodaySeconds = computed(() => footprint.todaySeconds)')
    expect(view).toContain('const displayWeekSeconds = computed(() => footprint.weekSeconds)')
    expect(view).toContain('const displayMonthSeconds = computed(() => footprint.monthSeconds)')
    expect(view).toContain('const displayTotalSeconds = computed(() => footprint.totalSeconds)')
    expect(view).not.toContain('const addPendingListenSeconds =')
    ...
    expect(view).toContain('const REMOTE_SNAPSHOT_INTERVAL_MS = 60_000')
    expect(view).toContain("safeRequest(listenTodaySongs(), '刷新今日听歌')")
    ...
    expect(view).toContain("if (activeTab.value === 'footprint' && !refreshing.value)")
