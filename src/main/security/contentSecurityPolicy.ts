...
const APP_DOCUMENT_URLS = ['http://localhost:41830/*', 'http://127.0.0.1:41830/*']

/*
 * Vue I18n 使用 9.3+ 的 JIT AST 编译模式，不再通过 Function 构造翻译表达式，
 * 因此 script-src 可以恢复为严格的 self-only 策略。
 *
...
const installOnSession = (targetSession: Session): void => {
  ...
  targetSession.webRequest.onHeadersReceived({ urls: APP_DOCUMENT_URLS }, (details, callback) => {
