...
const chacha20 = (key, counter, nonce, data) => {
  ...
  for (let off = 0; off < data.length; off += 64) {
    const keystream = chachaBlock(key, (counter + (off >>> 6)) >>> 0, nonce)
    const end = Math.min(off + 64, data.length)
    for (let i = off; i < end; i++) out[i] = data[i] ^ keystream[i - off]
  }
  return out
}
...
const encryptNCBL = (meta, body, opts = {}) => {
  ...
  for (let off = 0; off < compressed.length || off === 0; off += maxFrame) {
    ...
    frameHeader.writeUInt32LE(seq >>> 0, 2)
...
const buildMultipart = (payload) => {
  ...
  const fileName = `op_${Math.floor(Math.random() * 90000) + 10000}_0_${Math.floor(
...
const doUpload = async (ctx, metaJson, body, cookieStr) => {
  ...
  const success =
    respBody?.code === 200 && respBody?.data?.successfiles?.includes?.(fileName)
